...
// Build a smooth, single-curve path between two nodes, touching each circle edge.
function arcPath(fromId: string, toId: string): { d: string; length: number } {
  ...
  // Start and end on the circle edges so the line visually connects to each node
  const x1 = a.x + nx * a.r;
  const y1 = a.y + ny * a.r;
  const x2 = b.x - nx * b.r;
  const y2 = b.y - ny * b.r;
